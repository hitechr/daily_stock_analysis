...
5. **风险优先级**：舆情中的风险点要醒目标出"""
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if gemini_key_valid:
            try:
                ...
                logger.warning(f"Gemini 初始化失败: {e}，尝试 OpenAI/Anthropic 兼容 API")
                self._init_openai_fallback()
                if not self._openai_client:
                    self._init_anthropic_fallback()
        else:
            # Gemini Key 未配置，尝试 OpenAI 或 Anthropic
            logger.info("Gemini API Key 未配置，尝试使用 OpenAI/Anthropic 兼容 API")
            self._init_openai_fallback()
            if not self._openai_client:
                self._init_anthropic_fallback()

        # 所有 Provider 都未配置
        if not self._model and not self._openai_client and not self._anthropic_client:
    ...
    def _init_anthropic_fallback(self) -> None:
        ...
        try:
            ...
            logger.info(f"Anthropic API 初始化成功 (base_url: {config.anthropic_base_url}, model: {config.anthropic_model})")
        except Exception as e:
            error_msg = str(e).lower()
            if 'socks' in error_msg or 'socksio' in error_msg or 'proxy' in error_msg:
                logger.error(f"Anthropic 代理配置错误: {e}，如使用 SOCKS 代理请运行: pip install httpx[socks]")
            else:
    ...
    def is_available(self) -> bool:
        """检查分析器是否可用"""
        return self._model is not None or self._openai_client is not None or self._anthropic_client is not None
    ...
    def _call_anthropic_api(self, prompt: str, generation_config: dict) -> str:
        ...
        for attempt in range(max_retries):
            try:
                ...
                error_str = str(e)
                is_rate_limit = '429' in error_str or 'rate' in error_str.lower() or 'quota' in error_str.lower()
                is_overloaded = '500' in error_str or '503' in error_str or '负载' in error_str

                if is_rate_limit:
                    logger.warning(f"[Anthropic] API 限流，第 {attempt + 1}/{max_retries} 次尝试: {error_str[:100]}")
                elif is_overloaded:
                    logger.warning(f"[Anthropic] 服务繁忙，第 {attempt + 1}/{max_retries} 次尝试: {error_str[:100]}")
                else:
    ...
    def _call_api_with_retry(self, prompt: str, generation_config: dict) -> str:
        ...
        优先级：Gemini > Gemini 备选模型 > OpenAI 兼容 API > Anthropic API

        处理 429 限流错误：
        1. 先指数退避重试
        2. 多次失败后切换到备选模型
        3. Gemini 完全失败后尝试 OpenAI
        4. OpenAI 也失败后尝试 Anthropic
        ...
        if self._openai_client:
            ...
            try:
                ...
                if self._anthropic_client:
                    logger.warning("[OpenAI] 失败，尝试 Anthropic API")
                    try:
                        return self._call_anthropic_api(prompt, generation_config)
        ...
        # 尝试 Anthropic API
        if self._anthropic_client:
            logger.warning("[Gemini/OpenAI] 不可用，切换到 Anthropic API")
            try:
                return self._call_anthropic_api(prompt, generation_config)
            except Exception as anthropic_error:
                logger.error(f"[Anthropic] API 也失败: {anthropic_error}")
                raise last_error or anthropic_error
        elif config.anthropic_api_key:
            # 尝试懒加载初始化 Anthropic
            logger.warning("[Gemini/OpenAI] 不可用，尝试初始化 Anthropic API")
            self._init_anthropic_fallback()
            if self._anthropic_client:
                try:
                    return self._call_anthropic_api(prompt, generation_config)
